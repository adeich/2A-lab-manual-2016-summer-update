...
\label{act7.1.1}
\note{For Activity~\ref{act7.1.1} (\about\unit[70]{min})}{
...
\note{Look back on the Overview Sheet for the goals of this Activity}{\begin{itemize}\item Encourage the students to be gentle with the carts and tracks.  We want them to last.\item IMPORTANT!!!  Students need to be made to describe each collision in words, i.e., describe what happens, then perform a complete analysis for each case. \item In the discussion, you might spend a few minutes reviewing the summary on page 59 of the notes.   Make the analogy of $\vec{J}$ to work.   \item Heads up:  \FNTs{} will be based on what they are doing in this activity.
\end{itemize}
}
\begin{enumerate}
	...
	\note{\ref{711,1}:}{Make sure students say in words what physically changes when they alter $\Sigma \vec{F}$, $\Delta t$, $m$ or $\Delta v$.  Make sure their choices are physically possible.  If their example is to kick a ball and they tell you they could change the impulse by changing $\Delta t$, make sure they know how this could be done.}
	\begin{enumerate}
...
\end{enumerate}
\note{\ref{711,3}:}{
During the discussion, emphasize that all macroscopic collisions are inelastic to some degree.  Make sure they get that when things stick together, the collision is always inelastic but there are cases where they bounce off one another that are inelastic too.
}	
...
%	\item When total kinetic energy is conserved in a collision, we call it an elastic collision. If it is not conserved, we call it an inelastic collision. Classify the two collisions above as elastic or inelastic.
	\item \label{711,3}For each of the two collisions above, is the total kinetic energy conserved?
...
%	\item Does an inelastic collision violate conservation of energy?  If not, which other energy systems could the kinetic energy have gone to?
	\item \label{711,4} Does a case in which total kinetic energy is not conserved violate the \emph{conservation of energy} law? If not, which other energy systems could the kinetic energy have gone to?
	\note{\ref{711,4} and \ref{711,5} :}{
It is important that the students understand that momentum conservation and energy conservation are separate independent ideas.
}	
%	\item Is momentum conserved in the inelastic case and the elastic case?  Where in your \pchart{} is this shown?
	\item \label{711,5} Is momentum conserved in the case where total kinetic energy is conserved and in the case where total kinetic energy is not conserved? Where in your \pchart{} is this shown?
\end{enumerate}
\note{}{The students should start with $\vec{p}_{i} + \Delta \vec{p} = \vec{p}_{f}$ and then $m \vec{v_{i}} + m \Delta \vec{v} = m \vec{v_{f}}$.  For the case where the final speed is 0, make sure they substitute this in.}
\WCD
...
\label{act7.1.1b}
\note{For Activity~\ref{act7.1.1b} (\about\unit[70]{min})}{
We are using these charts to organize their thoughts and aid in their analysis for solving problems and answering questions that involve a change of motion.  The charts are used in a way that is similar to how we used energy interaction diagrams for the myriad of phenomena in \DLM~\ref{dlm7a}.  But, we cannot transfer our conservation diagrams directly since momentum is a vector, not a scalar.  Be emphatic that the momentum vectors in these charts are properly scaled and labeled.  Sloppiness will prevent the diagrams from being useful.  

Check to make sure that their equations match the vectors they have in their charts and stress the need to include force diagrams to find the impulse and thus $\Delta \vec{p}$.
}
