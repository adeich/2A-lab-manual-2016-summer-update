...
\begin{tikzpicture}
	...
	\node[align=left] at (3.95,0) {$\Delta E_\text{thermal} = m C_p \Delta T$\\\scriptsize{$m$: mass of substance}\\[-1.2ex]\scriptsize{$C_p$: Specific Heat of substance}};
\end{tikzpicture}\\
...
\begin{tikzpicture}
	...
	\node[align=left] at (5.35,0) {$\Delta E_\text{bond} = \Delta m H_p$\\\scriptsize{$\Delta m$: change in mass of substance in ``higher'' phase}\\[-1.2ex]\scriptsize{$H_p$: Heat of $\langle$Phase Change$\rangle$}};
\end{tikzpicture}
